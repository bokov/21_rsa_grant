...
.junk<-capture.output(fapar3 <- fa.parallel(select(dat3tr,all_of(v(c_domainexpert)))
                                            ,fm='ml',fa='fa'
                                            ,nfactors = length(v(c_domainexpert))
                                            ,show.legend=F));
...
fa3 <-fa(select(dat3tr,all_of(v(c_domainexpert))),nfactors = fapar3$nfact
